...
- **Cursed items** (`rogue/items.py`, `Game.use_item`): a negative weapon/armor enchantment is always cursed; rings have an independent 15% curse chance regardless of their effect. A cursed item is identified (and reveals a "(cursed)" tag) the moment it's equipped, and blocks wielding/wearing/putting on something else in that slot, and dropping, until uncursed. A Scroll of Enchant Weapon/Armor clears the curse on that slot; a Scroll of Remove Curse clears everything currently equipped at once.
- **Monster special abilities** (`rogue/monsters_data.py`, `Game._monster_attacks`): six species carry a `special` key resolved on hit (or every turn, for Troll's regen). Leprechaun and Nymph replace their normal attack entirely with a steal-then-teleport-away, and never take the Amulet or a cursed item that's stuck to you anyway.
